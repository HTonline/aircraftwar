...
var timer0 = null;   //第一个定时器名子
var timer1 = null;   //第二个定时器名子
...
function isCrash() {
	...
	for (var i = bianliang.enemyList.length - 1; i >= 0; i--) {
		//遍历了所有的敌人飞机，让所有的敌人飞机都与玩家机做一次检测，看是否有发生碰撞
		var result = checkCrash(bianliang.enemyList[i], hero);
		if (result) {
			//说明玩家飞机与敌人飞机发生了碰撞，那么，游戏结束
			//通过闹钟的名子，找到定时器，然后关闭掉这个定时器
